"""Unit tests for the utils module in the shared python package.

This module contains pytest tests for utility functions used across samples.
The repo root is added to sys.path early so first-party imports resolve during
test execution; the shared package is imported via importlib after the path is
updated so pylint still sees all import statements at the top of the module.
"""
...
# Ensure the repo root is on sys.path so imports for first-party packages resolve
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..')))

# Import the utils module after ensuring sys.path contains the repo root. We use
# importlib to avoid placing an import statement after executable code (which
# would trigger C0413). pylint will still see module imports at the top.
utils = importlib.import_module('shared.python.utils')

# Some tests use string-based monkeypatch targets like 'utils._get_project_root'.
# To ensure those references resolve, register the imported module under the
# simple name 'utils' in sys.modules so monkeypatch can find and patch it.
...
def test_cleanup_infra_deployments_thread_color_assignment(monkeypatch):
    ...
    _assigned_colors = [call[3] for call in cleanup_calls]
...
def test_extract_json_large_object():
    ...
    # json is available at module level
...
def test_print_functions_comprehensive():
    ...
    # io and sys are available at module level
...
def test_determine_policy_path_filename_mode(monkeypatch):
    ...
    # inspect and Path are available at module level
...
def test_generate_signing_key_format():
    ...
    # base64 available at module level
    try:
...
def test_run_command_with_error_suppression(monkeypatch):
    ...
    # subprocess available at module level
...
def test_cleanup_functions_comprehensive(monkeypatch):
    ...
    # apimtypes.INFRASTRUCTURE available at module level
...
def test_infrastructure_notebook_helper_create_with_index_retry(monkeypatch):
    ...
    # apimtypes enums available at module level
...
def test_infrastructure_notebook_helper_create_user_cancellation(monkeypatch):
    ...
    # apimtypes enums and pytest are available at module level
