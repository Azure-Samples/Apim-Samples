...
def test_check_azure_providers_registered_missing():
    ...
    with patch('shutil.which') as mock_which:
        with patch('subprocess.run') as mock_run:
            mock_which.return_value = '/usr/bin/az'
            mock_run.return_value = Mock(stdout='["Microsoft.Storage"]', returncode=0)
            result = sps.check_azure_providers_registered()
            assert result is False
...
def test_install_jupyter_kernel_locked_install_requires_uv(monkeypatch: pytest.MonkeyPatch):
    """Test install_jupyter_kernel refuses an unlocked pip fallback."""
    call_count = [0]
...
def test_force_kernel_consistency_with_existing_trusted(temp_project_root: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    existing_settings = {'jupyter.kernels.trusted': ['/custom/python/path']}

    (vscode_dir / 'settings.json').write_text(json.dumps(existing_settings), encoding='utf-8')
...
def test_check_azure_providers_all_providers_in_list():
    ...
    with patch('shutil.which') as mock_which:
        with patch('subprocess.run') as mock_run:
            ...
            mock_run.return_value = Mock(stdout=json.dumps(all_providers), returncode=0)
            result = sps.check_azure_providers_registered()
            assert result is True
...
def test_generate_env_file_preserves_custom_vars_order():
    ...
    try:
        ...
        (temp_project_root / '.env').write_text('CUSTOM_VAR1=value1\nCUSTOM_VAR2=value2\n', encoding='utf-8')
...
def test_check_azure_providers_prints_missing_count():
    ...
    with patch('shutil.which') as mock_which:
        with patch('subprocess.run') as mock_run:
            ...
            mock_run.return_value = Mock(stdout='["Microsoft.Storage", "Microsoft.Network"]', returncode=0)
            result = sps.check_azure_providers_registered()
...
def test_create_vscode_settings_preserves_jupyter_kernels():
    ...
    try:
        ...
        try:
            ...
            existing = {'other.setting': 'preserved'}
...
def test_force_kernel_consistency_settings_merge(temp_project_root: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    existing = {'python.analysis.exclude': ['custom/**']}
    (vscode_dir / 'settings.json').write_text(json.dumps(existing), encoding='utf-8')
...
def test_create_vscode_settings_merge_with_excludes():
    ...
    try:
        ...
        try:
            ...
            # Existing settings with custom excludes
            existing = {'python.analysis.exclude': ['custom/**', 'other/**']}
            (vscode_dir / 'settings.json').write_text(json.dumps(existing), encoding='utf-8')
...
def test_validate_kernel_setup_found():
    """Test validate_kernel_setup when kernel is found."""
    with patch('subprocess.run') as mock_run:
        mock_run.return_value = Mock(stdout=f'Available kernels:\n  {sps.KERNEL_NAME}\n', returncode=0)
        result = sps.validate_kernel_setup()
        assert result is True
...
def test_create_vscode_settings_existing_file_merge():
    ...
    try:
        ...
        try:
            ...
            existing_settings = {'python.linting.enabled': True, 'editor.fontSize': 14}
            (vscode_dir / 'settings.json').write_text(json.dumps(existing_settings), encoding='utf-8')
