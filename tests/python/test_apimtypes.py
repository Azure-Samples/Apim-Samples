...
class TestAPICreation:
    ...
    @pytest.mark.parametrize('tags', [['tag1', 'tag2'], ['single-tag'], ['foo', 'bar', 'baz']])
    ...
    @pytest.mark.parametrize('product_names', [['product1', 'product2'], ['single-product'], ['p1', 'p2', 'p3']])
    ...
    @pytest.mark.parametrize('missing_field', ['name', 'displayName', 'path', 'description'])
...
class TestAPIToDictSerialization:
    ...
    def base_api(self):
        ...
        return API(name='test-api', displayName='Test API', path='/test', description='A test API.', policyXml='<policies />')
...
class TestAPIComparisons:
    ...
    def sample_api(self):
        ...
        return API(name='test-api', displayName='Test API', path='/test', description='A test API.', policyXml='<policies />', tags=['a', 'b'])

    def test_equality_same_attributes(self, sample_api):
        """Test equality comparison for identical API objects."""
        api2 = API(name='test-api', displayName='Test API', path='/test', description='A test API.', policyXml='<policies />', tags=['a', 'b'])
        assert sample_api == api2

    @pytest.mark.parametrize('changed_attr,new_value', [('name', 'other-api'), ('tags', ['x']), ('productNames', ['different-product'])])
...
class TestEnums:
    ...
    @pytest.mark.parametrize('sku', [APIM_SKU.BASICV2, APIM_SKU.STANDARDV2, APIM_SKU.PREMIUMV2])
    ...
    @pytest.mark.parametrize(
        ...
        [(APIMNetworkMode, 'invalid'), (APIM_SKU, 'invalid'), (HTTP_VERB, 'FOO'), (INFRASTRUCTURE, 'bad'), (Region, 'notaregion')],
    )
...
class TestAPIOperation:
    ...
    def test_basic_operation_to_dict(self):
        ...
        op = APIOperation(name='op1', displayName='Operation 1', urlTemplate='/foo', method=HTTP_VERB.GET, description='desc', policyXml='<xml/>')
        d = op.to_dict()
    ...
    def test_get_operation2(self):
        ...
        op = GET_APIOperation2(name='get-users', displayName='Get Users', urlTemplate='/users', description='Get all users', policyXml='<custom/>')
    ...
    def test_invalid_method(self):
        ...
        with pytest.raises(ValueError):
            APIOperation(name='bad', displayName='Bad', urlTemplate='/bad', method='INVALID', description='desc', policyXml='<xml/>')

    def test_operation_accepts_string_method(self):
        """Test APIOperation accepts valid HTTP verb strings."""

        op = APIOperation(
            name='string-method', displayName='String Method', urlTemplate='/items', method='GET', description='desc', policyXml='<xml/>'
        )
...
class TestProductCreation:
    ...
    def base_product_params(self):
        ...
        return {'name': 'hr', 'displayName': 'Human Resources', 'description': 'HR product description'}
    ...
    @pytest.mark.parametrize(
        'state,subscription_req,approval_req', [('published', True, False), ('notPublished', False, False), ('published', True, True)]
    )
    def test_creation_with_custom_values(self, base_product_params, state, subscription_req, approval_req):
        ...
        product = Product(
            **base_product_params, state=state, subscriptionRequired=subscription_req, approvalRequired=approval_req, policyXml=custom_policy
        )
...
class TestPolicyFragment:
    ...
    def test_basic_creation(self):
        ...
        pf = PolicyFragment(name='Test-Fragment', policyXml='<policy>test</policy>', description='Test fragment')
...
class TestNamedValue:
    ...
    def test_creation(self, is_secret):
        ...
        nv = NamedValue(name='test-key', value='test-value', isSecret=is_secret)
...
class TestProductPolicyHandling:
    ...
    def test_product_with_none_policy_loads_default(self, monkeypatch):
        ...
        product = Product(name='test', displayName='Test', description='Test', policyXml=None)
    ...
    def test_product_with_explicit_policy_not_overridden(self):
        ...
        product = Product(name='test', displayName='Test', description='Test', policyXml=custom_policy)
...
class TestAPIOperationEdgeCases:
    ...
    def test_get_operation_with_template_parameters(self):
        ...
        op = GET_APIOperation(description='Get operation', policyXml='<policy/>', templateParameters=params)
...
class TestProductEdgeCases:
    ...
    def test_product_state_values(self):
        ...
        for state in ['published', 'notPublished']:
            product = Product(name='test', displayName='Test', description='Test', state=state)
            assert product.state == state
...
class TestAPIEdgeCases:
    ...
    def test_api_service_url(self):
        ...
        api = API(name='backend-api', displayName='Backend API', path='/backend', description='Backend API', serviceUrl=service_url)
    ...
    def test_api_subscription_required(self):
        ...
        api = API(name='public-api', displayName='Public API', path='/public', description='Public API', subscriptionRequired=False)
...
class TestOutputGetEdgeCases:
    ...
    def test_output_get_json_with_syntax_error(self):
        ...
        json_text = json.dumps({'output1': {'value': 'invalid syntax {bracket'}})
    ...
    def test_output_get_json_with_non_dict_properties(self):
        """Test Output.getJson() when properties is not a dict."""
        json_text = json.dumps({'properties': 'not a dict'})
        output = apimtypes.Output(success=True, text=json_text)
