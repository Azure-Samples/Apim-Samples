...
try {
	...
	# Since we have many tests, we omit the verbosity (-v). We also show the top 3 test durations that run slower than 0.2s as that is often an indicator of missing or faulty mocks.
	pytest --color=yes --durations=3 --durations-min=0.2 --cov --cov-config=tests/python/.coveragerc --cov-report=html:tests/python/htmlcov --cov-report=xml:coverage.xml --cov-report=json:coverage.json tests/python/
