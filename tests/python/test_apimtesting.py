...
def test_print_summary_all_passed():
    ...
    with patch('builtins.print') as mock_print:
        ...
        success_messages = [msg for msg in calls if 'ALL TESTS PASSED' in msg]
        assert len(success_messages) > 0

        # Should contain statistics
        stats_messages = [msg for msg in calls if 'Tests Passed' in msg]
...
def test_print_summary_some_failed():
    ...
    with patch('builtins.print') as mock_print:
        ...
        failure_messages = [msg for msg in calls if 'SOME TESTS FAILED' in msg]
        assert len(failure_messages) > 0

        # Should contain error details
        error_messages = [msg for msg in calls if 'Detailed Error Analysis' in msg]
...
def test_print_summary_no_tests():
    ...
    with patch('builtins.print') as mock_print:
        ...
        no_tests_messages = [msg for msg in calls if 'NO TESTS EXECUTED' in msg]
...
def test_print_summary_success_rate_calculation():
    ...
    calls = [c.args[0] for c in mock_print.call_args_list if c.args]
    success_rate_messages = [msg for msg in calls if '60.0%' in msg]
...
def test_print_summary_with_none_values():
    ...
    calls = [c.args[0] for c in mock_print.call_args_list if c.args]
    na_messages = [msg for msg in calls if 'N/A' in msg]
