...
- Verifies artifact release ages and installs locked Python packages via `uv sync --locked`
...
az extension add --name containerapp --version 1.3.0b4 --only-show-errors &&
az extension add --name front-door --version 2.2.0 --only-show-errors
```
...
```bash
az extension add --name containerapp --version 1.3.0b4
az extension add --name front-door --version 2.2.0
```
