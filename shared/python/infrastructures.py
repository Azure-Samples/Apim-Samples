...
import requests
import utils
from apimtypes import (API, APIM_SKU, HELLO_WORLD_XML_POLICY_PATH,
                       INFRASTRUCTURE, APIMNetworkMode, GET_APIOperation,
                       PolicyFragment)
...
class Infrastructure:
        ...
        try:
            ...
            if acct.success and acct.json_data:
                ...
                principal_type_param = (
                    "User" if str(user_type).lower() == "user" else "ServicePrincipal"
                )
                ...
                if principal_type_param == "User":
                    ...
                    if user_name:
                        sp = utils.run(
                            f"az ad sp show --id {user_name} --query id -o tsv",
                            print_command_to_run=False,
                            print_errors=False,
                        )
                ...
                if object_id:
                    self.bicep_parameters["kvCertificateCreatorObjectId"] = {
                        "value": object_id
                    }
                    self.bicep_parameters["kvCertificateCreatorPrincipalType"] = {
                        "value": principal_type_param
                    }
        except Exception:
    ...
    def _define_policy_fragments(self) -> List[PolicyFragment]:
        ...
        self.base_pfs = [
            ...
            PolicyFragment(
                "AuthZ-Match-All",
                utils.read_policy_xml(
                    utils.determine_shared_policy_path("pf-authz-match-all.xml")
                ),
            ...
            PolicyFragment(
                "AuthZ-Match-Any",
                utils.read_policy_xml(
                    utils.determine_shared_policy_path("pf-authz-match-any.xml")
                ),
            ...
            PolicyFragment(
                "Http-Response-200",
                utils.read_policy_xml(
                    utils.determine_shared_policy_path("pf-http-response-200.xml")
                ),
            ...
            PolicyFragment(
                "Product-Match-Any",
                utils.read_policy_xml(
                    utils.determine_shared_policy_path("pf-product-match-any.xml")
                ),
            ...
            PolicyFragment(
                ...
                utils.read_policy_xml(
                    utils.determine_shared_policy_path("pf-remove-request-headers.xml")
                ),
    ...
    def _define_apis(self) -> List[API]:
        ...
        api_hwroot = API(
            ...
            "Hello World",
            "",
            "This is the root API for Hello World",
            operations=[api_hwroot_get],
        )
        self.base_apis = [api_hwroot]

        # Combine base APIs with infrastructure-specific ones
        self.apis = (
            self.base_apis + self.infra_apis if self.infra_apis else self.base_apis
        )
    ...
    def _verify_infrastructure(self, rg_name: str) -> bool:
        ...
        try:
            ...
            output = utils.run(
                f'az apim list -g {rg_name} --query "[0]" -o json',
                print_command_to_run=False,
                print_errors=False,
            )
            ...
            if output.success and output.json_data:
                ...
                # Get API count
                api_output = utils.run(
                    f'az apim api list --service-name {apim_name} -g {rg_name} --query "length(@)"',
                    print_command_to_run=False,
                    print_errors=False,
                )
                ...
                if api_output.success:
                    ...
                    if api_count > 0:
                        try:
                            ...
                            sub_output = utils.run(
                                f'az apim subscription list --service-name {apim_name} -g {rg_name} --query "[0].primaryKey" -o tsv',
                                print_command_to_run=False,
                                print_errors=False,
                            )
    ...
    def deploy_infrastructure(self, is_update: bool = False) -> "utils.Output":
        ...
        try:
            ...
            if output.success:
                ...
                if output.json_data:
                    apim_gateway_url = output.get(
                        "apimResourceGatewayURL",
                        "APIM API Gateway URL",
                        suppress_logging=True,
                    )
                    apim_apis = output.getJson(
                        "apiOutputs", "APIs", suppress_logging=True
                    )
...
class ApimAcaInfrastructure(Infrastructure):
    ...
    def _verify_infrastructure_specific(self, rg_name: str) -> bool:
        ...
        try:
            ...
            aca_output = utils.run(
                f'az containerapp list -g {rg_name} --query "length(@)"',
                print_command_to_run=False,
                print_errors=False,
            )
...
class AfdApimAcaInfrastructure(Infrastructure):
    ...
    def _define_bicep_parameters(self) -> dict:
        ...
        afd_params = {
            "apimPublicAccess": {
                "value": True
            },  # Initially true for private link approval
            "useACA": {
                "value": len(self.infra_apis) > 0 if self.infra_apis else False
            },  # Enable ACA if custom APIs are provided
        }
    ...
    def _verify_apim_connectivity(self, apim_gateway_url: str) -> bool:
        ...
        try:
            ...
            # `requests` is imported at module top-level; avoid importing inside function
            healthcheck_url = f"{apim_gateway_url}/status-0123456789abcdef"
            print(
                f"   Testing connectivity to health check endpoint: {healthcheck_url}"
            )
            ...
            if response.status_code == 200:
                ...
                print(
                    f"   ⚠️  APIM health check returned status code {response.status_code} (expected 200)"
                )
                return (
                    True  # Continue anyway as this might be expected during deployment
                )

        except Exception as e:
            print(f"   ⚠️  APIM connectivity test failed: {str(e)}")
            print(
                "   ℹ️  Continuing deployment - this may be expected during infrastructure setup"
            )
    ...
    def deploy_infrastructure(self, is_update: bool = False) -> Output:
        ...
        apim_service_id = output.get(
            "apimServiceId", "APIM Service ID", suppress_logging=True
        )
        apim_gateway_url = output.get(
            "apimResourceGatewayURL", "APIM Gateway URL", suppress_logging=True
        )
        ...
        if not self._approve_private_link_connections(apim_service_id):
            ...
            failed_output = utils.Output(success=False, text="Private link approval failed")
        ...
        if not self._disable_apim_public_access():
            ...
            # Create a failed output object
            failed_output = utils.Output(success=False, text="Failed to disable public access")
            return failed_output
    ...
    def _verify_infrastructure_specific(self, rg_name: str) -> bool:
        ...
        try:
            # Check Front Door
            afd_output = utils.run(
                f'az afd profile list -g {rg_name} --query "[0]" -o json',
                print_command_to_run=False,
                print_errors=False,
            )
            ...
            if afd_output.success and afd_output.json_data:
                ...
                try:
                    apim_output = utils.run(
                        f'az apim list -g {rg_name} --query "[0].id" -o tsv',
                        print_command_to_run=False,
                        print_errors=False,
                    )
                    if apim_output.success and apim_output.text.strip():
                        ...
                        pe_output = utils.run(
                            f'az network private-endpoint-connection list --id {apim_id} --query "length(@)"',
                            print_command_to_run=False,
                            print_errors=False,
                        )
