...
class ApimTesting:
    ...
    def __init__(
        self,
        test_suite_name: str = "APIM Tests",
    ...
    def verify(self, value: any, expected: any) -> bool:
        ...
        try:
            ...
            assert (
                value == expected
            ), f"Value [{value}] does not match expected [{expected}]"
            self.tests_passed += 1
    ...
    def print_summary(self) -> None:
        ...
        print(
            f"    • Tests Failed : {self.tests_failed:>5} {'❌' if self.tests_failed > 0 else ''}"
        )
        ...
        if self.errors and len(self.errors) > 0:
            test_completion_msg = (
                f"Test execution completed with {len(self.errors)} error(s)!"
            )
