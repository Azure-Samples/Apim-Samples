...
try:
    import winreg as WINREG  # Windows-only registry access; optional import
except Exception:
    WINREG = None
import subprocess
import sys
...
from utils import (get_infra_rg_name, print_error, print_message, print_ok,
                   print_warning)
...
class CertificateManager:
    ...
    @staticmethod
    def install_certificate_for_infrastructure(
        ...
        if infrastructure_type not in [
            ...
            print_error(
                f"Certificate installation is only supported for Application Gateway infrastructures. Got: {infrastructure_type}"
            )
            return False
        ...
        print(
            f"Installing certificate for {infrastructure_type.value} infrastructure (index {index})"
        )
        print(f"Resource Group: {resource_group_name}")
        print("")

        print("🔍 Step 1: Checking Root CA...")
        root_ca_result = _resolve("ensure_apim_samples_root_ca")(
        ...
        try:
            key_vault_name = _resolve("_get_key_vault_name")(
                resource_group_name, show_errors
            )
            if not key_vault_name:
                print_error("No Key Vault found in the resource group")
                print(
                    "   This usually means the infrastructure hasn't been deployed yet."
                )
                print(
                    "   Please deploy the infrastructure first, then run this certificate installation."
                )
                return False
            ...
            if upload_success:
                ...
                print(
                    "   Future infrastructure deployments will use CA-signed certificates"
                )
            else:
                print_warning(" Failed to upload Root CA to Key Vault")
                print(
                    "   Existing certificate will still work, but future deployments may use self-signed certificates"
                )

            print("")

            print("🔍 Step 3: Downloading and installing infrastructure certificate...")
            return _resolve("_download_and_install_certificate")(
                resource_group_name,
                infrastructure_type,
                index,
                "ag-cert",
    ...
    def remove_all_apim_certificates() -> bool:
        ...
        print_warning(
            "Removing all APIM-Samples certificates from local trust store..."
        )
    ...
    @staticmethod
    def _download_and_install_certificate(
        resource_group_name: str,
        infrastructure_type: INFRASTRUCTURE,
        index: int,
        cert_name: str = "ag-cert",
        ...
        try:
            ...
            if user_prompt:
                ...
                if is_ca_signed:
                    print_ok(
                        "Certificate is already CA-signed by APIM Samples Root CA!"
                    )
                    print(
                        "   No additional installation needed - your browser should trust it automatically."
                    )
                    return True
                else:
                    print_warning(
                        "Certificate appears to be self-signed (created with old template)"
                    )
                    print(
                        "   This means your infrastructure was deployed before the CA-based certificates were implemented."
                    )
                    print(
                        "   The certificate will still work, but you may see SSL warnings."
                    )
                    print("")
                    print("💡 To get CA-signed certificates:")
                    print("   1. The Root CA has been uploaded to your Key Vault")
                    print(
                        "   2. Redeploy your infrastructure to get a CA-signed certificate"
                    )
                    ...
                    try:
                        user_choice = (
                            input(
                                "🤔 Do you want to install the self-signed certificate now? (y/N): "
                            )
                            .strip()
                            .lower()
                        )
            ...
            cert_display_name = (
                "apim-samples"
                if not resource_group_name or resource_group_name.strip() == ""
                else f"apim-samples-{resource_group_name}"
            )
            ...
            success = install_func(
                cert_data, cert_display_name, infrastructure_type, index
            )
    ...
    def _get_key_vault_name(
        resource_group_name: str, show_errors: bool = True
    ) -> Optional[str]:
        ...
        output = run_func(
            f'az keyvault list -g {resource_group_name} --query "[0].name" -o tsv',
            print_errors=show_errors,
        )
    ...
    @staticmethod
    def _download_certificate_from_key_vault(
        key_vault_name: str, cert_name: str, resource_group_name: str
    ) -> Optional[bytes]:
        ...
        try:
            ...
            output = run_func(
                f"az keyvault secret download --vault-name {key_vault_name} --name {cert_name} --file {temp_path} --overwrite",
                print_errors=False,
            )
            ...
            if output.success and os.path.exists(temp_path):
                ...
                if "Forbidden" in output.text and "getSecret" in output.text:
                    print_warning(
                        "❌ Permission not yet propagated, waiting 10 seconds..."
                    )
                    time.sleep(10)
                    retry_output = run_func(
                        f"az keyvault secret download --vault-name {key_vault_name} --name {cert_name} --file {temp_path} --overwrite",
                        print_errors=False,
                    )
                    if retry_output.success and os.path.exists(temp_path):
                        with open(temp_path, "rb") as f:
                            cert_data = f.read()
                        print_ok(
                            f"Certificate downloaded after permission propagation ({len(cert_data)} bytes)"
                        )
                        return cert_data
                    else:
                        print_error(
                            "Failed to download certificate even after waiting for permission propagation"
                        )
                        return None
                else:
    ...
    @staticmethod
    def _grant_current_user_keyvault_access(
        key_vault_name: str, resource_group_name: str
    ) -> bool:
        try:
            run_func = _resolve("run")
            if not callable(run_func):
            ...
            user_output = run_func(
                "az ad signed-in-user show --query id -o tsv", print_errors=False
            )
            ...
            sub_output = run_func(
                "az account show --query id -o tsv", print_errors=False
            )
            ...
            role_output = run_func(
                f'az role assignment create --assignee "{user_object_id}" --role "Key Vault Secrets User" --scope "{scope}"',
                print_errors=False,
            )

            if role_output.success:
                print_ok(
                    "Successfully granted Key Vault Secrets User access to current user"
                )
                return True
            else:
                print_warning(
                    "Failed to grant automatic access (you may need Owner/Contributor permissions)"
                )
                return False
    ...
    @staticmethod
    def _install_certificate_to_trust_store(
        cert_data: bytes,
        display_name: str,
        infrastructure_type: INFRASTRUCTURE,
        index: int,
    ...
    def _install_certificate_windows_powershell(temp_path: str) -> bool:
        try:
            ...
            if result.returncode == 0 and "SUCCESS:" in result.stdout:
                print_ok(
                    "Certificate installed to Windows trusted root store (PowerShell)"
                )
                return True
            else:
                print_message(
                    f"PowerShell method failed: {result.stdout} {result.stderr}"
                )
                return False
    ...
    def _install_certificate_windows_certutil(temp_path: str) -> bool:
        try:
            ...
            if result.returncode == 0:
                print_ok(
                    "Certificate installed to Windows trusted root store (certutil)"
                )
                return True
            else:
                print_message(
                    f"Certutil method failed: {result.stdout} {result.stderr}"
                )
                return False
    ...
    def _install_certificate_macos(cert_data: bytes) -> bool:
        ...
        try:
            ...
            try:
                subprocess.run(extract_cmd, shell=True, capture_output=True, text=True, check=True)
            except subprocess.CalledProcessError as e:
                err_text = getattr(e, "stderr", None) or getattr(e, "output", "")
                print_error(f"Failed to extract certificate: {err_text}")
                return False
            ...
            try:
                ...
                err_text = getattr(e, "stderr", None) or getattr(e, "output", "")
                print_error(f"Failed to install certificate: {err_text}")
                return False
    ...
    def _install_certificate_linux(
        ...
        try:
            ...
            extract_result = subprocess.run(
                extract_cmd, shell=True, capture_output=True, text=True, check=True
            )
    ...
    def _remove_certificates_windows() -> bool:
        ...
        print("3. Look for certificates containing 'apim-samples'")
        print("4. Right-click and delete each one")
        return True

    @staticmethod
    def _remove_certificates_macos() -> bool:
        cmd = 'sudo security delete-certificate -c "apim-samples" /Library/Keychains/System.keychain'
        result = subprocess.run(cmd, shell=True, capture_output=True, text=True, check=False)
        ...
        if result.returncode == 0:
            ...
            print_warning(
                "No APIM-Samples certificates found to remove (or removal failed)"
            )
            return False
    ...
    @staticmethod
    def check_openssl_availability(refresh_path: bool = True) -> bool:
        if refresh_path:
            CertificateManager._refresh_path_from_environment()
        try:
            openssl_exec = _resolve("get_openssl_executable")
            if callable(openssl_exec):
                openssl_exec = openssl_exec()
            # Fallback to shutil.which if resolver didn't provide one
            if not openssl_exec:
                openssl_exec = shutil.which("openssl")

            if not openssl_exec:
                return False

            try:
                subprocess.run([openssl_exec, "version"], capture_output=True, text=True, check=True)
                return True
    ...
    def handle_missing_openssl() -> str:
        ...
        if system == "windows":
            print(
                "   • **Recommended**: winget install --id ShiningLight.OpenSSL.Light"
            )
            print(
                "   • Development version: winget install --id ShiningLight.OpenSSL.Dev"
            )
            ...
            print("")
            print(
                "   ⚠️  **Important**: After installation, you may need to add OpenSSL to your PATH:"
            )
...
def ensure_apim_samples_root_ca(interactive: bool = True) -> str:
    ...
    if system == "windows":
        check_cmd = (
            f'certutil -store "{cert_store}" | findstr /C:"APIM Samples Root CA"'
        )
    ...
    if system == "windows":
        ...
        print(
            "   https://raw.githubusercontent.com/Azure-Samples/api-management-samples/master/certificates/apim-samples-root-ca.cer"
        )
        ...
        print("   mkdir -p ~/.local/share/ca-certificates/apim-samples")
        print(
            "   cp ~/Downloads/apim-samples-root-ca.cer ~/.local/share/ca-certificates/apim-samples/"
        )
...
def upload_root_ca_to_key_vault(key_vault_name: str) -> bool:
    ...
    if not (
        getattr(cert_path, "exists", lambda: False)()
        and getattr(key_path, "exists", lambda: False)()
    ):
        ...
        try:
            ...
            try:
                subprocess.run(
                    f"curl -sSL {ca_cert_url} -o {ca_cert_path}",
                    shell=True,
                    capture_output=True,
                    text=True,
                    check=True,
    ...
    try:
        ...
        print_message(
            f"Uploading to Key Vault '{key_vault_name}' as secret '{secret_name}'..."
        )
        ...
        try:
            ...
            pfx_cmd = [
                ...
                str(cert_path),
                "-name",
                "APIM Samples Root Certificate",
                "-passout",
                f"pass:{DEFAULT_PFX_PASSWORD}",
            ]

            try:
                subprocess.run(pfx_cmd, capture_output=True, text=True, check=True)
            except subprocess.CalledProcessError as e:
                err_text = getattr(e, "stderr", None) or getattr(e, "output", "")
                print_error(f"Failed to create PFX file: {err_text}")
                return False
            ...
            cert_result = run_func(
                f'az keyvault certificate import --vault-name {key_vault_name} --name "apim-samples-root-ca" --file "{temp_pfx_path}" --password "{DEFAULT_PFX_PASSWORD}"'
            )
            ...
            if cert_result.success:
                ...
                err_text = getattr(cert_result, "stderr", None) or getattr(
                    cert_result, "text", ""
                )
...
def get_openssl_executable():
    """Return a best-effort path to the OpenSSL executable, or None if not found.

    Strategy:
    - Honor OPENSSL_PATH environment variable (file or directory).
    - Check common install locations for Windows/macOS/Linux.
    - Fall back to shutil.which('openssl').
    """
    try:
        ...
        if env_path:
            ...
            if p.is_file():
                return str(p)
            # If a directory was provided, check common binary names
            candidate = p / "openssl"
            if candidate.exists():
                return str(candidate)
            candidate_exe = p / "openssl.exe"
        ...
        if system == "windows":
            ...
            candidates = ["/usr/bin/openssl", "/usr/local/bin/openssl"]
...
def ensure_apim_samples_root_ca_auto() -> bool:
    ...
    """
    # ------------------------------
    #    ROOT CA SCENARIOS (explicit)
    # ------------------------------
    # The code below must handle four possible states regarding the Root CA:
    #
    # Scenario 1: Local files exist (cert + key) AND Root CA is installed in
    #             the OS trust store.
    #   - Desired action: Nothing to do; return success. The existing files and
    #     store entry are authoritative.
    #
    # Scenario 2: Local files exist (cert + key) BUT Root CA is NOT installed
    #             in the OS trust store.
    #   - Desired action: Create a PFX from the local cert+key and import it
    #     into the OS trust store (platform-specific helpers). If import
    #     succeeds, return success. If import fails, surface failure.
    #
    # Scenario 3: Local files are MISSING BUT Root CA IS installed in the OS
    #             trust store.
    #   - Desired action: Try to export the certificate (and private key, if
    #     exportable) from the OS store to local files. If export succeeds,
    #     return success. If export fails and OpenSSL is available, recreate
    #     local cert/key (regenerate) and optionally re-install; otherwise
    #     return failure or defer to interactive/manual steps.
    #
    # Scenario 4: Local files are MISSING AND Root CA is NOT installed in the
    #             OS trust store.
    #   - Desired action: If OpenSSL is available, create local cert+key and
    #     then install into the trust store. If OpenSSL is not available,
    #     return False (indicating we couldn't complete the automatic setup).
    #
    # Implementation notes:
    # - Helper functions are dynamically resolved via _resolve so tests can
    #   patch behavior. Key helpers used below:
    #     * get_root_ca_paths() -> (ca_dir, cert_path, key_path)
    #     * _is_root_ca_installed() -> bool
    #     * check_openssl_availability() -> bool
    #     * _create_root_ca() -> bool
    #     * _install_root_ca() -> bool
    #     * _import_local_root_ca_to_store(cert_path, key_path) -> bool
    #     * _export_root_ca_from_store_to_local(cert_path, key_path) -> bool
    ...
    try:
        ...
        # Scenario 1 & 2: Local files exist
        if cert_path.exists() and key_path.exists():
            # Scenario 1: local files exist and Root CA is already installed
            try:
                if _is_root_ca_installed():
                    print_message(
                        "Scenario 1: Local files exist and Root CA already installed -> nothing to do"
                    )
                    return True
            except Exception:
                # On failure of the installed check, fall through to attempt import
                pass

            # Scenario 2: local files exist but Root CA not installed -> try import
            print_message(
                "Scenario 2: Local files exist but Root CA not installed -> attempting import into store"
            )
        ...
        # Scenario 3: files missing but Root CA is installed in store
        if callable(is_installed_func) and is_installed_func():
            # Try to export from the store to local files
            if not (cert_path.exists() and key_path.exists()):
                export_func = _resolve("_export_root_ca_from_store_to_local")
                if callable(export_func):
                    try:
                        print_message(
                            "Scenario 3: Local files missing but Root CA present in store -> attempting export from store to local files"
                        )
                        exported = export_func(cert_path, key_path)
                        if exported:
                            print_message("Scenario 3: Export succeeded")
                            return True
                    except Exception:
                        # ignore export errors and fall back to regeneration
                        print_message(
                            "Scenario 3: Export failed; will attempt regeneration if OpenSSL available"
                        )

            # If export did not succeed, try regeneration if OpenSSL is available
            if callable(openssl_func) and openssl_func():
                print_message(
                    "Regenerating Root CA files and installing (OpenSSL available)"
                )
        ...
        # Scenario 4: files missing and Root CA not installed in store -> create + install if OpenSSL available
        if callable(openssl_func) and openssl_func():
            print_message(
                "Scenario 4: Local files missing and Root CA not installed -> creating Root CA and installing (OpenSSL available)"
            )
            if callable(create_func):
                create_func()
            if callable(install_func):
...
def upload_root_ca_for_infrastructure(
    infrastructure_type: INFRASTRUCTURE, index: int
) -> bool:
...
def _is_root_ca_installed() -> bool:
    ...
    try:
        print_message("_is_root_ca_installed: certificate module loaded")
    except Exception:
        pass
...
def _is_root_ca_installed_windows() -> bool:
    try:
        ...
        if result.returncode == 0 and (
            "APIM Samples Root" in (result.stdout or "")
            or "apim-samples" in (result.stdout or "").lower()
        ):
            return True
...
def _is_root_ca_installed_macos() -> bool:
    try:
        ...
        return result.returncode == 0 and "APIM Samples Root CA" in (result.stdout or "")
    except Exception:
        return False
...
def _export_root_ca_from_store_to_local(cert_path: Path, key_path: Path) -> bool:
    ...
    try:
        ...
        find_result = subprocess.run(
            find_cmd, shell=True, capture_output=True, text=True, check=False
        )
        ...
        if not thumbprint:
            print_message(
                "Could not locate a suitable APIM Samples Root certificate in the store for export"
            )
            return False
        ...
        try:
            subprocess.run(export_cmd, shell=True, capture_output=True, text=True, check=True)
        except subprocess.CalledProcessError as e:
            err_text = getattr(e, "stderr", None) or getattr(e, "output", "")
            print_message(f"Failed to export PFX from store: {err_text}")
            try:
        ...
        try:
            ...
            try:
                subprocess.run(extract_cert_cmd, shell=True, capture_output=True, text=True, check=True)
                subprocess.run(extract_key_cmd, shell=True, capture_output=True, text=True, check=True)
            except subprocess.CalledProcessError as e:
                err_text = getattr(e, "stderr", None) or getattr(e, "output", "")
                print_message(f"OpenSSL extraction failed: {err_text}")
                return False
...
install_certificate_for_infrastructure = (
    CertificateManager.install_certificate_for_infrastructure
)
...
_download_certificate_from_key_vault = (
    CertificateManager._download_certificate_from_key_vault
)
_grant_current_user_keyvault_access = (
    CertificateManager._grant_current_user_keyvault_access
)
_install_certificate_to_trust_store = (
    CertificateManager._install_certificate_to_trust_store
)
...
_install_certificate_windows_powershell = (
    CertificateManager._install_certificate_windows_powershell
)
_install_certificate_windows_certutil = (
    CertificateManager._install_certificate_windows_certutil
)
