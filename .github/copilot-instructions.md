...
- Enforce an eight-day post-release waiting period for every package ecosystem. Python resolution must use the `tool.uv.exclude-newer = "8 days"` setting, locked installs must run `python setup/verify_dependency_age.py --scope python` before `uv sync --locked`, and GitHub Actions must pass the repository-wide dependency-age workflow.
...
- Keep Dependabot `cooldown.default-days` at eight or greater for every configured ecosystem. Security updates remain subject to the same repository verifier even when the hosting service bypasses cooldown for advisory PRs.
