"""
This module deploys the AG_APIM_PE infrastructure (App Gateway in front of APIM via Private Endpoint).
"""

import sys
import argparse
from apimtypes import APIM_SKU, INFRASTRUCTURE
from infrastructures import AgApimPeInfrastructure
import utils


def create_infrastructure(location: str, index: int, apim_sku: APIM_SKU) -> None:
    try:
        infrastructure_exists = utils.does_resource_group_exist(utils.get_infra_rg_name(INFRASTRUCTURE.AG_APIM_PE, index))
        result = AgApimPeInfrastructure(location, index, apim_sku).deploy_infrastructure(infrastructure_exists)
...
def main():
    parser = argparse.ArgumentParser(description = 'Create AG_APIM_PE infrastructure')
