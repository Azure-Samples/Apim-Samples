...
param apimSubnetPrefix string = '10.20.2.0/24'
    ...
    securityRules: [
      {
        ...
        properties: {
          ...
          sourcePortRange: '*'
          destinationPortRange: '443'
          // Restrict ingress strictly to the App Gateway subnet address prefix
...
// Key Vault and self-signed certificate for HTTPS
...
resource kvRoleAssignment 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  name: guid(subscription().id, keyVault.id, uami.name, '4633458b-17de-408a-b874-0445c86b69e6')
  scope: keyVault
  properties: {
...
// RBAC: assign Key Vault Certificates Officer to UAMI for deployment script
...
// Optional: assign Key Vault Certificates Officer to the deploying principal to allow certificate creation
...
// Self-signed certificate creation using deployment script (more reliable than direct ARM resource)
resource kvCertScript 'Microsoft.Resources/deploymentScripts@2023-08-01' = {
  ...
  properties: {
    ...
    scriptContent: '''
      ...
      # Use a simpler approach: Import a self-signed certificate directly
...
// 4. Virtual Network and subnets (with delegations)
module vnetModule '../../shared/bicep/modules/vnet/v1/vnet.bicep' = {
  ...
  params: {
    ...
    subnets: [
      // App Gateway Subnet (dedicated, no delegations)
      {
      ...
      // APIM Subnet (External VNet mode delegation)
      {
        name: apimSubnetName
        properties: {
...
// 5. APIM (External VNet mode, public enabled; Developer SKU per requirement)
...
// 6. Application Gateway with backend to APIM public hostname
